...
def dir2(obj='default', second=object, unique_only=False, consistent_only=False, parameter='', print_output=True, show='all', exclude_external_modules=False):
    ...
    for identifier in identifiers_to_examine:
        if exclude_external_modules == True:
            if identifier in sys.modules:        
                    try:
                        module_name = obj.__file__.rsplit('\\')[-2]
                        module_file = getattr(obj, identifier).__file__
                        if module_name not in module_file:
                            identifiers_to_examine.remove(identifier) 
                    except ImportError:
                        identifiers_to_examine.remove(identifier) 
