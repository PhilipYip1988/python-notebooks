...
There is no start menu shortcut for JupyterLab and it needs to be launched using the Mambaforge Prompt. Activate the Python environment using:
...
The Python environment will change from (base) to (jupyterlab). Input the CMD command jupyter followed by the option lab:
...
There is also an extensions tab... previously this was used to install and remove extensions however this should typically be done using mamba and their associated packages from the ```conda-forge``` channel. ```jupyterlab-variableinspector``` is an example of an installed extension.
...
The PowerShell terminal is similar to what the Mambaforge PowerShell Prompt should be allowing PowerShell commands such as the mamba package manager and python. Notitce the jupyterlab Python environment is selected:
